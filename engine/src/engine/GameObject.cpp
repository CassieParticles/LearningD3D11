...
void GameObject::updateTransformationMatrix()
{
	...
	window->getDeviceContext()->Map(transformationCBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &transformationMap);

	memcpy(transformationMap.pData, &mat, sizeof(mat));

	window->getDeviceContext()->Unmap(transformationCBuffer.Get(), 0);
